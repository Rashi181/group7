...
// ── DataReduction helpers ──────────────────────────────────────────────────
void Graph::dr_add_neighbor(uint32_t u, uint32_t w, uint32_t weight) {
    adj[u].push_back({w, weight});
}
 
void Graph::dr_record_bag(uint32_t v, const std::vector<uint32_t>& neighbors) {
    td_bag_edges[v].push_back({v, 0});
    for (auto nb : neighbors)
        td_bag_edges[v].push_back({nb, get_edge_weight(nb, v)});
}
 
void Graph::dr_remove_vertex(uint32_t v, const std::vector<uint32_t>& neighbors) {
    for (auto nb : neighbors) {
...
void Graph::dr_remove_isolated(uint32_t v, std::vector<uint32_t>& ordering) {
    adj.erase(v);
    num_vertices--;
    td_bag_edges[v].push_back({v, 0});
...
void Graph::populate_buckets_min_fill_no_edge_init() {
    // Edge cache already populated — just seed buckets with fill scores
    for (const auto& [u, edges] : adj) {
        uint32_t fill_num = get_fill(u);
        buckets[fill_num].push_back(u);
        bucket_position[u] = std::prev(buckets[fill_num].end());
...
std::tuple<Graph::TreeDecompAdj, Graph::TreeDecompBags, uint32_t> Graph::get_td(Heuristic heuristic, float alpha) {
    ...
    if (heuristic == Heuristic::MIN_FILL) {
    ...
    } else if (heuristic == Heuristic::MIN_FILL_REDUCTION) {
        // Populate edge cache first (needed by both data reduction and min-fill)
        for (const auto& [u, edges] : adj)
            for (const auto& [to, w] : edges)
                h.add_edge_cache(u, to, w);
 
        // Phase 1: eliminate cheap vertices before min-fill
        h.td_bag_edges.resize(adj_size);
        std::vector<uint32_t> reduced;
        DataReduction::apply(h, reduced);
        std::cout << "  [dr] eliminated " << reduced.size()
                  << " vertices, " << h.adj.size() << " remaining" << std::endl;
 
        // Assign ordering positions for reduced vertices
        std::vector<uint32_t> ordering(adj_size, UINT32_MAX);
        for (size_t i = 0; i < reduced.size(); i++) {
            uint32_t v = reduced[i];
        ...
        // Phase 2: min-fill on remaining graph (edge cache already populated)
        h.populate_buckets_min_fill_no_edge_init();
 
        // Store reduced ordering offset so main loop continues from right position
        h.adj_original_size = reduced.size(); // reuse field to pass offset
    ...
    if (heuristic == Heuristic::LEX_BFS) {
        ...
        // Reduced vertices already have ordering/bags set — continue from their count
        size_t offset = 0;
        for (size_t i = 0; i < adj_size; i++)
            if (ordering[i] != UINT32_MAX) offset++;
 
        size_t remaining = h.adj.size();
        for (size_t i = 0; i < remaining; i++) {
            ...
            if (remaining > 0 && i % std::max<size_t>(1, remaining / 10) == 0)
                std::cout << "  min-fill " << 100 * i / remaining << "%" << std::endl;
        }
    } else {
        for (size_t i = 0; i < adj.size(); i++) {
            uint32_t v = h.pop_next_vertex();
